...
fn validated_path(raw: &str, write: bool) -> Result<PathBuf, String> {
    ...
    if !path.is_absolute() {
        return Err(
            "TRAMA solo acepta rutas absolutas seleccionadas mediante un diálogo nativo."
                .to_string(),
        );
    }
    if path
    ...
    if write {
        if !matches!(extension.as_str(), "trama" | "svg" | "png" | "pdf") {
            return Err(
                "TRAMA solo puede escribir archivos .trama, .svg, .png o .pdf.".to_string(),
            );
        }
        let parent = path
            .parent()
            .ok_or_else(|| "La ruta no tiene carpeta de destino.".to_string())?;
        if !parent.is_dir() {
        ...
        if extension != "trama" {
            return Err(
                "TRAMA solo puede abrir archivos .trama desde el diálogo nativo.".to_string(),
            );
        }
        if !path.is_file() {
    ...
    tauri::Builder::default()
        ...
        .menu(|app| {
            ...
            let save_as_item = MenuItemBuilder::with_id("save-as", "Guardar como .trama…")
                ...
                .build(app)?;
            let export_item = MenuItemBuilder::with_id("export", "Exportar…").build(app)?;
            ...
            let redo_item = MenuItemBuilder::with_id("redo", "Rehacer")
                ...
                .build(app)?;
            let settings_item =
                MenuItemBuilder::with_id("settings", "Privacidad y preferencias…").build(app)?;
            let about_item = MenuItemBuilder::with_id("about", "Acerca de TRAMA").build(app)?;
